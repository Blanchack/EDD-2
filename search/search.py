...
data_linear = pd.read_csv('linear_search.csv', names=['N', 'Time'])
data_quadratic = pd.read_csv('quadratic_search.csv', names=['N', 'Time'])
data_double = pd.read_csv('double_search.csv', names=['N', 'Time'])
data_unordered = pd.read_csv('unordered_map_search.csv', names=['N', 'Time'])
data_abierto = pd.read_csv('abierto_search.csv', names=['N', 'Time'])
...
def add_plot(ax, data, avg_data, title):
    ...
    ax.set_xlabel('Nro. de Busqueda')
    ax.set_ylabel('Tiempo (nanosegundos)')
    ax.set_title(f'Analisis de tiempo de Busqueda ({title})')
    ax.legend()
...
plt.tight_layout()
fig.savefig('combined_data_with_averages.png')
...
def add_table(ax, avg_data, title):
    ...
    col_labels = ['Nro. de Busqueda', 'Tiempo promedio (ns)']
...
plt.tight_layout()
fig_tables.savefig('average_data_tables.png')
